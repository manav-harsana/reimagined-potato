...
echo "NOTE: This Script only runs as root, make sure you have already partitioned and mounted all the Filesystems to their respective places"
echo "Distro: Artix"
echo "Init: Runit"
echo "Shell: Fish"
echo "AUR Helper: yay"
...
echo "Installing Packages from main artix/arch repositories..."
pacman -S xorg tar git neofetch linux-zen linux-zen-headers base-devel go cinnamon mousepad nemo geary mpv wget cronie nano curl lynx dhcpcd dhcpcd-runit efibootmgr fish flatpak gimp grub sddm pulseaudio networkmanager network-manager-applet wine-staging wine-gecko winetricks wine-mono wine-nine unzip blueman --noconfirm
...
cd yay && makepkg -si
cd ..
echo "Yay Compilation Complete!"
echo "Creating user and settings Passwords..."
useradd -m -G wheel -s /bin/fish manav
echo "Choose Password for manav"
passwd manav
echo "Password Set Successfully!"
echo "Choose Password for Root"
passwd
echo "Password Set Successfully!"	
echo "Switching User to Manav.."
su manav
echo "Installing Packages from AUR..."
yay -S alacritty-git bluez-tools-git cbonsai hollywood flatseal g4music pamac-nosnap light-git librewolf-bin pfetch sddm-sugar-candy-git skippy-xd-git st starship-git ttf-d2coding --noconfirm
echo "Packages Installed from AUR!"
exit
echo "Setting Up SDDM..."
cp config/sddm.conf /etc/
sv up sddm
echo "SDDM setup complete!"
echo "Setting Up Desktop themes and prompts..."
su manav
mkdir -p ~/.config/fish && mkdir -p ~/.config/alacritty && mkdir -p ~/.themes && mkdir -p ~/.icons
exit
cp config/config.fish /home/manav/.config/fish/ && cp config/alacritty.toml /home/manav/.config/alacritty/ && cp -r icons/* /home/manav/.icons/ && cp -r themes/* /home/manav/.themes/
su manav
gsettings set org.cinnamon.theme name \'Orchis-Teal-Dark\'
gsettings set org.cinnamon.desktop.interface icon-theme \'Tela-circle-manjaro-dark\'
gsettings set org.cinnamon.desktop.interface gtk-theme \'Graphite-Dark\'
exit
echo "Themes and Configuration Applied!"
